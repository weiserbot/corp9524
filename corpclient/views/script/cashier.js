...
function createCashier(form) {
	fetch('http://localhost:3000/cashiers/createcashier', {
		method: "POST",
...
async function readCashiers() {
	await fetch('http://localhost:3000/cashiers/readcashiers')
		.then(res => res.json())
		.then(data => {
			displayTable(data);
		});
...
function earlyDeposit(form) {
	if (checkEarlyDeposit == "0" && checkCashierStatus == "open") {
		...
		fetch('http://localhost:3000/cashiers/updateearlydeposit', {
			method: "PUT",
			headers: { "Content-Type": "application/json" },
...
function closeCashier() {
	if (checkCashierStatus == "open") {
		const cashierForm = document.getElementById('cashier-form');
		fetch('http://localhost:3000/cashiers/closecashier', {
			method: "PUT",
			headers: { "Content-Type": "application/json" },
...
async function runCashierUpdate() {
	if (checkCashierStatus == "open") {
		const cashierform = document.getElementById('cashier-form');
		await fetch('http://localhost:3000/cashiers/runcashierupdate', {
			method: "PUT",
			headers: { "Content-Type": "application/json" },
